...
//            mBluetoothAdapter.getBluetoothLeScanner().stopScan(mLeScanCallback);
    ...
    public void setCharacteristicNotification(BluetoothGattCharacteristic characteristic,
        ...
        if (UUID_HEART_RATE_MEASUREMENT.equals(characteristic.getUuid())) {
            ...
            Log.e("properties", "descriptor" + "=" + descriptor.toString());
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
            ...
            int writeLength = value.length * 2;

            String write_over = null;
            try {
                write_over = bin2hex(new String(characteristic.getValue(),"utf-8"));
            } catch (UnsupportedEncodingException e) {
                e.printStackTrace();
            }
            Log.e("write_over",write_over);
            //写数据结束发送广播
            if (write_over != null){
                if (writeLength>30&&!write_over.endsWith("584D")) {
                    broadcastUpdate(WRITE_COMPLETE,writeLength+"");
                    Log.e("发送广播",WRITE_COMPLETE+"--"+writeLength);
                }
            }
        }
